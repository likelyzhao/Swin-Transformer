...
class TokenLearnerV11(nn.Module):
    def __init__(self, in_c, num_tokens, input_resolution, drop=0.):
        super().__init__()
        self.in_c = in_c
    ...
    def forward(self, x):
        selected = x
        batch = selected.shape[0] // self.input_resolution[0]//self.input_resolution[1]
        selected = self.norm(selected)
        selected = torch.transpose(selected, 0,1)
        ...
        selected = selected.reshape(-1, batch, selected.shape[1], selected.shape[2])
        ...
        feat = feat.reshape(-1, batch, feat.shape[1],  feat.shape[2], feat.shape[3])
        feat = feat.reshape(feat.shape[0], batch, feat.shape[3]*feat.shape[4], -1)
...
class TokenLearnerV11windows(nn.Module):
    ...
    def forward(self, x):
        ...
        #batch = selected.shape[0] // self.input_resolution[0]//self.input_resolution[1]
        selected = self.norm(selected)
        selected = torch.transpose(selected, -2,-1)
        #selected = selected.reshape(selected.shape[0], -1, self.input_resolution[0], self.input_resolution[1])
        selected = selected.reshape(-1, self.in_c, self.input_resolution[0], self.input_resolution[1])
        selected = self.groupconv_select(selected)
        selected = self.conv1x1_select(selected)
...
class TokenFuser(nn.Module):
    def __init__(self, in_c, num_tokens, input_resolution, use_normalization=True, drop=0.):
        super().__init__()
        self.in_c = in_c
        self.num_tokens = num_tokens
        self.use_normalization = use_normalization
        self.input_resolution= input_resolution
        self.norm_original = nn.LayerNorm([self.in_c, self.input_resolution[0], self.input_resolution[1]])
    ...
    def forward(self, inputs, original):
        ...
        inputs = inputs.reshape(inputs.shape[0], -1, self.num_tokens, inputs.shape[2])
        ...
        original = original.reshape(-1, self.in_c,  self.input_resolution[0], self.input_resolution[1])
        original = self.norm_original(original)
        mix = self.conv1x1_mix(original)
        mix = mix.reshape(feat_dim, -1, mix.shape[-3], self.input_resolution[0]*self.input_resolution[1])
        mix = torch.transpose(mix, -1, -2)
        mix = self.sigmoid(mix)
        mix = torch.transpose(mix, -1, -2)
        mix = mix.reshape(mix.shape[0], mix.shape[1], self.input_resolution[0], self.input_resolution[1], -1)
        mix = torch.unsqueeze(mix, -1)

        inputs = inputs * mix 
        inputs = torch.sum(inputs, dim =-2)

        inputs = inputs.reshape(inputs.shape[0], inputs.shape[1], self.input_resolution[0]*self.input_resolution[1], -1)
        inputs = inputs.reshape(inputs.shape[0], -1, inputs.shape[-1])
        inputs = torch.transpose(inputs, 0, 1)
        return self.drop(inputs)


class TokenFuserWindows(nn.Module):
    def __init__(self, in_c, num_tokens, windows_size, use_normalization=True, drop=0.):
        super().__init__()
        self.in_c = in_c
        self.num_tokens = num_tokens
        self.use_normalization = use_normalization
...
class WindowAttentionMod(nn.Module):
    ...
    def __init__(self, dim, num_token, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):

        super().__init__()
        self.dim = dim
        self.num_token = num_token  # Wh, Ww
        ...
        self.relative_position_bias = nn.Parameter(
            torch.zeros((num_token), (num_token), num_heads))  # 2*Wh-1 * 2*Ww-1, nH
    ...
    def forward(self, x, mask=None):
        ...
        relative_position_bias = self.relative_position_bias.permute(2, 0, 1).contiguous()  # nH, Wh*Ww, Wh*Ww
    ...
    def extra_repr(self) -> str:
        return f'dim={self.dim}, num_token={self.num_token}, num_heads={self.num_heads}'
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.mlp = Mlp(in_c=dim, in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
...
class SwinTransformerBlockMod(nn.Module):
    r""" Swin Transformer Block.
    ...
    def forward(self, x):
        ...
        residual = x_windows
        x_windows = self.learner(x_windows)
        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows, mask=self.attn_mask)  # nW*B, window_size*window_size, C

        attn_windows = self.fuser(attn_windows, residual)
        attn_windows = residual + attn_windows
...
class BasicLayerFuse(nn.Module):
    """ A basic Swin Transformer layer for one stage.
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False, num_token=8):
        ...
        # build blocks
        self.blocks = nn.ModuleList([
            SwinTransformerBlockMod(dim=dim, input_resolution=input_resolution,
                                 num_heads=num_heads, window_size=window_size,
                                 shift_size=0 if (i % 2 == 0) else window_size // 2,
                                 mlp_ratio=mlp_ratio,
                                 qkv_bias=qkv_bias, qk_scale=qk_scale,
...
class SwinTransformerLearn(nn.Module):
    r""" Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.layers = nn.ModuleList()
        for i_layer in range(self.num_layers):
            layer = BasicLayerFuse(dim=int(embed_dim * 2 ** i_layer),
                                    input_resolution=(patches_resolution[0] // (2 ** i_layer),
                                                        patches_resolution[1] // (2 ** i_layer)),
                                    depth=depths[i_layer],
                                    num_heads=num_heads[i_layer],
...
if __name__ == "__main__":
    ...
    print("params = ", params/1024/1024 , " MB")
